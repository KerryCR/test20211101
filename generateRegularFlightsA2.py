...
RegularFlightsA_xls = xlrd.open_workbook('RegularFlightsA.xls')  # xls
RegularFlightsATable = RegularFlightsA_xls.sheet_by_name('TimeRanking')  # Time-Ranked Table
...
for i in range(0, n):
    for j in range(0, n):
        ...
        if tmp1 == tmp2:
            Pnn[i][j] = 1
# print(type(Pnn), Pnn)
Pdn = [0 for i in range(0, n)]  # 生成出发地标志列表Pdn，pdi表示航班i的出发地是否为基地NKX
for i in range(0, n):
    if str(Sd[i]) == str(Sd[0]):
        Pdn[i] = 1
Pdn = tuple(Pdn)
# print(type(Pdn), Pdn)
Pan = [0 for i in range(0, n)]  # 生成到达地标志列表Pan，pai表示航班i的到达地是否为基地NKX
for i in range(0, n):
...
FirstFlight = m.addVars(NL, 1, lb=0, ub=n-1, vtype=GRB.INTEGER, name="FirstFlight")    # 添加NL*1变量，命名为FirstFlight,存放每条航线第一班序号
DutyTime = m.addVars(NL, 1, vtype=GRB.INTEGER, name="DutyTime") # 添加NL*1变量，命名为DutyTime 存放航线总时长
temp_n = range(0, n)
temp_NL = range(0, NL)
...
for i in temp_NL:
    for j in temp_n:
        for k in range(j, n):
            m.addConstrs(quicksum(l[m, i] for m in range(0, j)) + quicksum(l[n, j] for n in range(k, n)) + DutyTime[i, 0] + int(Ta[k].value) - int(Td[j].value) == 0, "DutyTime")
...
obj = m.getObjective()
print(obj.getValue())
m.write("outputFlightsA.sol")

# output_RegularFlightA = xlwt.Workbook()
# sheet1 = output_RegularFlightA.add_sheet(u'sheet1', cell_overwrite_ok=True)  # 创建sheet
# iout = 0
# for it in x:
#     for jout in range(len(it)):
#         sheet1.write(iout, jout, str(it[jout]))
#     iout = iout + 1
# output_RegularFlightA.save(".")
...
# for i in temp_NL:
#     for j in range(0, n-1):
#         m.addConstr(l[j, i] - 2 * sum(l[k, i] for k in range(j+1, n)) <= 10000 * Pdn[j], "basea{i}{j}")
# # 航线内航班地点互相衔接的约束
# for i in temp_NL:
#     for j in temp_n:
#         for k in range(j+1, n):
#             m.addConstr(l[j, i] + l[k, i] - 1 - 10000 * sum(l[m, i] for m in range(j+1, k)) - 10000 * Pnn[j][k] <= 0, "P{i}{j}")
# # 航线内航班时间相互衔接的约束
# for i in temp_NL:
#     for j in temp_n:
#         for k in range(j+1, n):
#             m.addConstr(l[j, i] + l[k, i] - 1 - 10000 * sum(l[m, i] for m in range(j+1, k)) - 10000 * Tnn[j][k] <= 0, "T{i}{j}")
# m.optimize()
# m.printStats()
# x = m.getVars()  # 决策变量值
# for it in x:
